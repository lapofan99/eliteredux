...
u32 AI_GetMoveAccuracy(u8 battlerAtk, u8 battlerDef, u16 atkAbility, u16 defAbility, u8 atkHoldEffect, u8 defHoldEffect, u16 move)
{
    ...
    if (defAbility == ABILITY_BAD_LUCK || GetBattlerAbility(BATTLE_PARTNER(battlerDef)) == ABILITY_BAD_LUCK || SpeciesHasInnate(gBattleMons[battlerDef].species, ABILITY_BAD_LUCK, gBattleMons[battlerAtk].level))
    ...
    // Check Wonder Skin.
    if ((defAbility == ABILITY_WONDER_SKIN || SpeciesHasInnate(gBattleMons[battlerDef].species, ABILITY_WONDER_SKIN, gBattleMons[battlerAtk].level)) && gBattleMoves[move].power == 0)
        moveAcc = 50;
...
static bool32 PartyBattlerShouldAvoidHazards(u8 currBattler, u8 switchBattler)
{
    ...
    if (ability == ABILITY_MAGIC_GUARD || ability == ABILITY_LEVITATE || SpeciesHasInnate(GetMonData(mon, MON_DATA_SPECIES), ABILITY_LEVITATE, GetMonData(mon, MON_DATA_LEVEL))
...
void IncreaseStatUpScore(u8 battlerAtk, u8 battlerDef, u8 statId, s16 *score)
{
    ...
    if (AI_DATA->atkAbility == ABILITY_CONTRARY || (BattlerHasInnate(battlerDef, ABILITY_UNAWARE) && statId != STAT_SPEED) || (AI_DATA->defAbility == ABILITY_UNAWARE && statId != STAT_SPEED))
        return;
